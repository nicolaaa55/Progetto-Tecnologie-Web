...
const { fetchRandomWikipediaArtistArticle } = require("../services/wikipedia");
  ...
  try {
    ...
    const leaderboard = Object.values(statsByUser).map((player) => ({
      ...
      averageTimeSeconds: Math.round(
        player.totalTimeSeconds / player.wonMatchesCount,
      ),
    }));
...
router.get("/completed", optionalAuthenticateToken, async (req, res) => {
  try {
    ...
    const formattedMatches = completedMatches.slice(0, 50).map((m) => {
    ...
    res
      .status(500)
      .json({ error: "Errore nel caricamento delle partite concluse." });
  }
});
  ...
  try {
    ...
    if (
      ...
      return res.json({
        ...
        maskedText: maskText(
          activeMatch.originalText,
          activeMatch.guessedWords || [],
        ),
      });
    }
    ...
    if (!artistData || !artistData.text) {
      return res
        .status(500)
        .json({ error: "Errore nel recupero dati da Wikipedia" });
    }
...
router.post("/:id/guess", optionalAuthenticateToken, async (req, res) => {
  try {
    ...
    if (typeof guess !== "string" || !guess.trim()) {
      return res
        .status(400)
        .json({ error: "Il tentativo (guess) è obbligatorio." });
    }
